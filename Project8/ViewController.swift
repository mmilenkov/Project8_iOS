    ...
    @objc func submitTapped(_ sender: UIButton) {
        ...
        if let solutionPosition = solutions.firstIndex(of: answer) {
            ...
            if(solutions.count == 0) {
                let ac = UIAlertController(title: "Well done!", message: "Are you ready to go for the next level", preferredStyle: .alert)
                ac.addAction(UIAlertAction(title: "Let's go!", style: .default, handler: levelUp))
                present(ac,animated: true)
            }
        } else {
            let ac = UIAlertController(title: "Incorrect", message: "That answer is not correct", preferredStyle: .alert)
            ac.addAction(UIAlertAction(title: "Clear", style: .default, handler: clear))
            present(ac, animated: true)
        }
    }
